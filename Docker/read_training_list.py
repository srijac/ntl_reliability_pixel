...
def create_training_zarrs():
    f=pd.read_csv('A33_poweroutage_training.csv',header=0)
    for i in np.arange(0,len(f)):
        h=f.iloc[i,5]
        v=f.iloc[i,6]
        tile_base_lat_top=90-(v-0)*10
        tile_base_lon_west=180-(h-0)*10
        if v>=9:
            tile_base_lat_top=-tile_base_lat_top
        if h<=18:
            tile_base_lon_west=-tile_base_lon_west
        lat_top=tile_base_lat_top-f.iloc[i,7]
        lat_bottom=tile_base_lat_top-f.iloc[i,9]
        lon_west=tile_base_lon_west-f.iloc[i,8]
        lon_east=tile_base_lon_west-f.iloc[i,10]
        d=f.iloc[i,4]
        mdy=d.split('-')
        m=mdy[0]
        d=mdy[1]
